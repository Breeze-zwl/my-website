<template>
  <n-card :bordered="false" class="header" content-style="padding: 0">
    <section>
      <div class="logo" @click="router.push('/Provider/home')">
          ...
          <n-popover>
            <template #trigger>
              <n-button
                ...
                strong
                round
                @click="router.push('/Provider/setting')"
              >
                <template #icon>
...
import { DailyGetCurrentTime } from '@/utils/DailyGetCurrentTime.js'
...
onMounted(() => {
  window.$timeInterval = timeInterval.value = setInterval(() => {
    store.timeData = DailyGetCurrentTime()
  }, 1000)
})
